...
describe("Auction contract testing", function () {
  ...
  let tokenIds: [number, number, number, number];
  ...
  beforeEach(async () => {
    ...
    tokenIds = [140, 70, 30, 12];
  });

  it("Should revert if one of the token has an auction running or ended", async function () {
    await auction.startAuctions(tokenIds);
    await expect(auction.startAuctions(tokenIds)).revertedWith("A selected token has an auction active or ended");
  });
  ...
  it("Should emit event after starting auctions", async function () {
    await expect(await auction.startAuctions(tokenIds)).emit(auction, "AuctionStarted");
  });
  ...
  it("Should revert if caller of withdraw function is not the contract owner", async function () {
    await expect(auction.connect(user1).withdrawAll()).revertedWithCustomError(auction, "OwnableUnauthorizedAccount");
  });
  ...
  it("Should revert if placing bid on an a token which hasn't an auction active", async function () {
    await expect(auction.placeBid(1)).revertedWith("Auction on this token isn't running");
  });

  it("Should revert if placing bid on an a token which auction has ended", async function () {
    await auction.startAuctions(tokenIds);
    await networkHelpers.time.increase(100000000);
    await expect(auction.placeBid(140)).revertedWith("Auction ended");
  });

  it("Should revert if placing bid on an a token which user has alredy the highest bid", async function () {
    await auction.startAuctions(tokenIds);
    await auction.placeBid(140, { value: ethers.parseEther("1") });
    await expect(auction.placeBid(140, { value: ethers.parseEther("2") })).revertedWith("You already have the highest bid");
  });

  it("Should revert if bid isn't superior to starting price", async function () {
    await auction.startAuctions(tokenIds);
    await expect(auction.placeBid(140, { value: 1000 })).revertedWith("Bid must be equal or superior to starting price");
  });

  it("Should revert if bid proposed on an auction isn't superior to highest bid", async function () {
    await auction.startAuctions(tokenIds);
    await auction.placeBid(140, { value: ethers.parseEther("1") });
    await expect(auction.connect(user1).placeBid(140, { value: ethers.parseEther("0.1") })).revertedWith(
  ...
  it("Should refund overtaken bid to its bidder", async function () {
    ...
    await auction.connect(user1).placeBid(140, { value: ethers.parseEther("2") });
  ...
  it("Should register bid value and address when received", async function () {
    await auction.startAuctions(tokenIds);
    await auction.placeBid(140, { value: ethers.parseEther("1") });
    let a = await auction.tokenAuction(140);
  ...
  it("Should emit event after starting auctions", async function () {
    await auction.startAuctions(tokenIds);
    await expect(auction.placeBid(140, { value: ethers.parseEther("1") })).emit(auction, "NewBidPlaced");
  });

  it("Should revert if trying to end auctions before its expiring time", async function () {
    await auction.startAuctions(tokenIds);
    await expect(auction.endAuctions(tokenIds)).revertedWith("Auctions not expired yet");
  });
  ...
  it("Should emit event after ending auctions", async function () {
    await auction.startAuctions(tokenIds);
    await networkHelpers.time.increase(100000000);
    await expect(auction.endAuctions(tokenIds)).emit(auction, "AuctionsClosed");
  });

  it("Should delete array after closing auctions", async function () {
    await auction.startAuctions(tokenIds);
    await networkHelpers.time.increase(100000000);
    await auction.endAuctions(tokenIds);
    for (let i = 0; i < tokenIds.length; i++) {
      expect(await auction.activeAuctionsTokenIds(i)).equal(0n);
    }
  });

  it("Should revert if tying buy directly a token whitch isn't unsold", async function () {
    await expect(auction.buyUnsold(1)).revertedWith("Token selected ins't unsold");
  });
  ...
  it("Should emit event after purchasing an unsold NFT", async function () {
    ...
    await networkHelpers.time.increase(100000000);
    await auction.endAuctions(tokenIds);
    await expect(auction.buyUnsold(140, { value: ethers.parseEther("1") })).emit(auction, "BoughtUnsoldNFT");
  });
